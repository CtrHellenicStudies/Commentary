	 ...
	 barGraphBars
				.on('click', (d) => {
					if (d.nComments) {
						...
						var foundSubwork = self.props.work.subworks.find((element, index, array) => {
							return element.n === self.state.selectedBar
						});
						var numberOfLines = Math.floor(d3.max(foundSubwork.commentHeatmap, (d) => {
								return d.n;
						})/10);
		...
		return (
			<div className={`work-teaser work-teaser--${work.slug}`}>
				<div className="commentary-text">
					...
					<div
						className={`text-subworks text-subworks-visualization-${work.slug}`}
					/>
				</div>
			</div>
		);
