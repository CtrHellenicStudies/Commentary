...
export default ReferenceWorkContext = React.createClass({
	...
	getMeteorData() {
		if (!this.props.referenceWorkId) {
			return;
		}
		let lemmaText = [];
		const context = {};

		const commentsSub = Meteor.subscribe('comments.referenceWork_context', this.props.referenceWorkId);

		if (commentsSub.ready()) {
			const commentCursor = queryCommentWithKeywordId(this.props.referenceWorkId);
			...
			if (commentCursor.count() > 0) {
				...
				const textNodesSub = Meteor.subscribe('textnodes.referenceWork_context', textNodesQuery);
